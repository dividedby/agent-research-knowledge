...
- [workslop-is-an-asymmetric-effort-attack](./workslop-is-an-asymmetric-effort-attack.md) — pasted AI-generated text costs the sender nothing but the reader full effort; defend by refusing, closing the effort gap with your own AI, forcing a symmetric channel, or ignoring it.
- [ai-breaks-legible-proxies-for-skill](./ai-breaks-legible-proxies-for-skill.md) — once AI can cheaply produce a field's legible signal of expertise (a solved puzzle, a GitHub side project), the signal stops discriminating skill and the field must wall off human-only competition or find new, uncounterfeitable signals.
- [dont-build-separate-tools-for-agents](./dont-build-separate-tools-for-agents.md) — agents use computers the way humans do, so tools good for humans are already good for agents; a new agent-native tool must overcome the incumbent's training-data advantage, and today's real edge is just exposing plain text/APIs, not a redesign.
- [fast-models-need-fast-dev-loops](./fast-models-need-fast-dev-loops.md) — as inference speed rises, generation stops being the bottleneck and tool-call latency (file reads, test runs) becomes it, pushing agentic codebases toward fast-compiling languages and reviving DevEx as a discipline.
- [give-agents-priorities-not-just-a-task-spec](./give-agents-priorities-not-just-a-task-spec.md) — tell agents your broader goals and relative priorities, not just the literal task; modern-model mistakes come from wrong assumptions about intent, not confusion, and a literal spec forecloses improvements only context can surface.
- [validate-with-a-general-classifier-then-distill](./validate-with-a-general-classifier-then-distill.md) — prototype a classification feature with a promptable general model; production usage generates the labeled dataset for free, making the later swap to a cheap bespoke classifier close to free too.
